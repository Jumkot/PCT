...
void calculate_forces_parallel(struct particle* p, struct particle* f, float* m, int n_threads)
{
...
double nbody_parallel(int n_threads)
{
    ...
    for (double t = 0; t <= 1; t += dt)
    { // Цикл по времени (модельному)
        calculate_forces_parallel(p, f, m, n_threads); // Вычисление сил – O(N^2)
        move_particles(p, f, v, m, dt); // Перемещение тел O(N)
    }
