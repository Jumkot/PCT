...
plot x title "Linear speedup" with lines lc rgb 'blue' lt 1 lw 2,\
     'data1.dat' using 1:2 title "N - body (одна критическая секция)" with linespoints ls 2,\
     'data2.dat' using 1:2 title "N - body (6 атомарных операций)" with linespoints ls 3,\
     'data3.dat' using 1:2 title "N - body (n блокировок)" with linespoints ls 4,\
     'data4.dat' using 1:2 title "N - body (избыточные вычисления без блокировок)" with linespoints ls 5,\
     'data5.dat' using 1:2 title "N - body (локальный вектор без блокировок)" with linespoints ls 6
